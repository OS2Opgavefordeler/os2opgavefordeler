	...
	function ExtendedResponsibilityController($log, $scope, $modalStack, topicRouterApi) {
		...
		function show(filter) {
			...
			topicRouterApi.getFiltersForRule($scope.topic.id).then(function (res) {
				...
				for (var j in $scope.orgUnits) {
					if ($scope.orgUnits[j].id == $scope.model.assignedOrgId) {
						setSelectedOrgUnit($scope.orgUnits[j]);
					}
				}

				for (var l in $scope.employments) {
					if ($scope.employments[l].id == $scope.model.assignedEmployeeId) {
						setSelectedEmp($scope.employments[l]);
					}
				}
			});
		}
